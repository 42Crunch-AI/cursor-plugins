...
below. Every credential field is collected via `AskQuestion` — never
...
- If none of the above match and the OAS provides no clear signals, call
  `AskQuestion`: `"I couldn't automatically detect any privileged operations.
...
Announce which endpoint will be used. Then collect credentials in sequence, grouped by user:

- **No BOLA found** — ask for User 1:
  - "What is User 1's username or email?" → store as `{{username}}`
  - "What is User 1's password or PIN?" → store as `{{password}}`

- **BOLA found** — ask for User 1, then User 2:
  - User 1: username/email → `{{username}}`, password/PIN → `{{password}}`
  - User 2: username/email (must NOT share User 1's resources) → `{{user2Username}}`, password/PIN → `{{user2Password}}`

For BFLA (admin) credentials, ask separately after the BOLA pair — collect `{{adminUsername}}` and `{{adminPassword}}` under an "Admin" heading in chat.

**Bearer / JWT** (no login endpoint in OAS):

- `AskQuestion`: `"I need a bearer token for User 1. Do you have one ready, or acquire from an endpoint?"` — options: `["I have a token — I'll paste it", "I need to acquire one — I'll specify the endpoint"]`
...
**API Key**: ask conversationally for the key value, store as `{{apiKey}}`. Header/param name from `securitySchemes[*].name` and `in`.

**Basic Auth**: use the same adaptive pattern as Login endpoint — 2 prompts (no BOLA) or 4 prompts (BOLA). For BFLA admin, ask separately under an "Admin" heading.

**OAuth2**: `AskQuestion`: `"Do you have an access token, or use the token endpoint from the OAS?"` — options: `["I have an access token", "Use the token endpoint — I'll provide client credentials"]`. Collect accordingly.
...
1. Ask conversationally: "Please share the path to your Postman collection file (v2.1 JSON format)." — wait for the file path.
...
Output the classification explanation and table above as a chat message, then call `AskQuestion`:
...
resource cannot be created in this environment), call `AskQuestion`:
...
user for explicit consent. Call `AskQuestion`:
...
and confirm when ready."`, then call a second `AskQuestion`:
...
Mandatory behavior:
- The very next action after completing 12b MUST be the `AskQuestion` call defined in this section for the active mode.
...
**Platform mode** — call `AskQuestion`:
...
**Free Trial mode** — call `AskQuestion`:
...
change for each fix one at a time in unified diff format then call `AskQuestion`:
...
If **"Show me the relevant code first"** is chosen, locate each handler (step 12e-3) and display the relevant code block without making any changes, then call `AskQuestion` again with the same options to proceed.
...
For each proposed code change, display it in unified diff format and call `AskQuestion`:
